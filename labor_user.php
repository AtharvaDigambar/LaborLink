...
<body>
    <header>
        ...
        <div class="hero">
            <h1>Find Skilled Laborers for Your Projects</h1>
            <p>Here is the list of skilled laborers:</p>
            <p class="blink">GET UPTO 30% OFF</p>
        </div>
    </header>

    <div class="container labor-list">
        <?php
        // Database connection
        ...
        $sql = "SELECT username, profession, gender, experience, labor_address, contact_number FROM labor_users";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            // Output data of each row
            while($row = $result->fetch_assoc()) {
                echo '<div class="labor-item">';
                echo '<h3>' . $row["username"] . '</h3>';
                echo '<p><strong>Profession:</strong> ' . $row["profession"] . '</p>';
                echo '<p><strong>Gender:</strong> ' . $row["gender"] . '</p>';
                echo '<p><strong>Experience:</strong> ' . $row["experience"] . '</p>';
                echo '<p><strong>Address:</strong> ' . $row["labor_address"] . '</p>';
